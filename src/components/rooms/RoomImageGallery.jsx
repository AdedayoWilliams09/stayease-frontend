...
/**
 * RoomImageGallery Component - Room-specific image gallery
 * 
 * @param {array} images - Array of image URLs
 * @param {string} roomName - Room name for alt text
 */
const RoomImageGallery = ({ images = [], roomName = 'Room' }) => {
  ...
  if (!images || images.length === 0) {
    return (
      <div className="w-full h-[350px] md:h-[450px] lg:h-[500px] bg-gray-200 dark:bg-gray-700 flex items-center justify-center rounded-2xl">
  ...
  const GalleryContent = () => (
    <div className="relative w-full h-[350px] md:h-[450px] lg:h-[500px] bg-gray-900 rounded-2xl overflow-hidden group">
      {/* Main Image */}
      ...
      <button
        onClick={openLightbox}
        className="absolute top-4 right-4 p-3 rounded-full bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm shadow-md hover:shadow-lg transition-all min-h-[44px] min-w-[44px] flex items-center justify-center"
        ...
        <>
          ...
          >
            ...
            <motion.img
              key={currentIndex}
              src={images[currentIndex]}
              alt={`${roomName} - Full screen image ${currentIndex + 1}`}
